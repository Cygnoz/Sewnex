...
>(undefined);
export const PosResponseContext = createContext<
  PosResponseContextType | undefined
>(undefined);
...
>(undefined);
export const UnitResponseContext = createContext<
  unitResponseContextType | undefined
>(undefined);
export const UnitEditResponseContext = createContext<
  unitEditResponseContextType | undefined
>(undefined);
export const TableResponseContext = createContext<
  TableLoadingContextType | undefined
>(undefined);
export const PreviousPathContext = createContext<
  PreviousPathContextType | undefined
>(undefined);
export const CustomerDeatilsContext = createContext<
  CustomerDeatilsContextType | undefined
>(undefined);
...
export const OCRInvoiceContext = createContext<OCRInvoiceContextType>({
  ocrInvoice: null,
  setOcrInvoice: () => {},
});
export const octAddItemContext = createContext<ocrAddItemContextType>({
  ocrAddItem: null,
  setOcrAddItem: () => {},
});
...
const ContextShare: React.FC<ContextShareProps> = ({ children }) => {
  ...
  const getSettingsData = async () => {
    try {
      const url = `${endpoints.GET_SETTINGS_DATA}`;
  ...
  return (
    ...
    >
      <CustomerDeatilsContext.Provider
        value={{ customerDatials, setCustomerDatials }}
      >
        <UnitEditResponseContext.Provider
          value={{ unitEditResponse, setEditUnitResponse }}
        >
          <UnitResponseContext.Provider
            value={{ unitResponse, setUnitResponse }}
          >
            <cashResponseContext.Provider
              value={{ cashResponse, setCashResponse }}
            >
              <BankResponseContext.Provider
                value={{ bankResponse, setBankResponse }}
              >
                <PosResponseContext.Provider
                  value={{ posResponse, setPosResponse }}
                >
                  <CurrencyResponseContext.Provider
                    value={{ currencyResponse, setCurrencyResponse }}
                  >
                    <GstResponseContext.Provider
                      value={{ gstResponse, setGstResponse }}
                    >
                              ...
                              >
                                ...
                                >
                                  <PreviousPathContext.Provider
                                    value={{ previousPath, setPreviousPath }}
                                  >
                                    <OCRInvoiceContext.Provider
                                      value={{ ocrInvoice, setOcrInvoice }}
                                    >
                                      <octAddItemContext.Provider
                                        value={{ ocrAddItem, setOcrAddItem }}
                                      >
