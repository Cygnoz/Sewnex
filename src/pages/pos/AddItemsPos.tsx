...
function AddItemsPos({ selectedItems, onRemoveItem, selectedCustomer }: Props) {
  ...
  const getSalesInvoicePrefix = async () => {
    try {
      const prefixUrl = `${endpoints.GET_INVOICE_PREFIX}`;
    ...
    <div className="bg-white p-6 mt-3 rounded-lg h-auto">
      <div>
        <div className="flex justify-between items-center">
          <p className="text-[#303F58] text-sm font-bold">Selected Item</p>
          <p className="text-[#4B5C79] text-sm font-semibold">Order No: {prefix}</p>
        </div>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center w-[60%]">
                  <img
                    ...
                    alt={item.itemName}
                  />
                  <p className="text-[#4B5C79] text-xs font-semibold ms-3">
                    {item.itemName}
                    <br />
                    <span className="text-[#303F58] font-bold text-xs block mt-1.5">
