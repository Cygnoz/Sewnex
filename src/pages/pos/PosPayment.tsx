...
function PosPayment({ selectedItems, total, selectedCustomer, selectedMethodLabel, quantities, discountType, discount, subtotal
  ...
  const handleSave = async () => {
    try {
      const url = `${endpoints.ADD_SALES_INVOICE}`;
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-[#4B5C79] font-bold text-sm mt-4">
          {selectedItems?.length} Items
        </p>
        <p className="text-[#4B5C79] font-bold text-base mt-2">Total </p>
        <p className="mt-2 font-bold text-2xl text-[#303F58]">
          ₹ {total.toFixed(2)}
        </p>

        {/* Input Field */}
        <div className="mt-4">
          <label className="text-[#303F58] text-sm">Paid</label>
          <input
            type="text"
