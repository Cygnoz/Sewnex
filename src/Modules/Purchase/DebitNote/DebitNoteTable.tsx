...
const DebitNoteTable = ({
  ...
  const removeRow = (index: number) => {
    if (rows.length > 1) {
      ...
      // Updatestate with the default row
      setState?.((prevData: any) => ({
        ...prevData,
  ...
  useEffect(() => {
    ...
    setState?.((prevData: any) => ({
      ...
      grandTotal: totalSellingPrice + state.totalTaxAmount,
    }));
  }, [rows, state.totalTaxAmount]);
  ...
  }, [isInterState, state?.destinationOfSupply, state?.sourceOfSupply]);
    ...
    <div>
      <div className="rounded-lg border-2 border-tableBorder mt-5">
        <table className="min-w-full bg-white rounded-lg relative pb-4 border-dropdownText">
          <thead className="text-[12px] text-center text-text_tertiary">
          ...
          <tbody className="text-text_tertiary text-center text-[13px] ">
            {rows?.map((row: any, index: number) => (
              <tr key={index}>
                <td className="py-2.5 px-4 border-y border-tableBorder text-gray-500">
                  {index + 1}
                </td>
